...
class Journal:
    ...
    def MakeJSON(self):
        dic = {'ID': self.id, 'Title': self.Title, 'ISSN': self.ISSN, 'Articles': list(self.Articles),
               'arthors': list(self.arthors)}
...
class Article:
    ...
    def MakeJSON(self):
        dic = {'ID': self.id, 'Title': self.title, 'Pmid': self.pmid, 'Date': self.date, 'Terms': self.terms,
               'Journal': self.Journal, 'Authors': list(self.authors)}
        JSON = json.dumps(dic, ensure_ascii=False)
        return JSON
...
class Author:
    ...
    def MakeJSON(self):
        dic = {'ID': self.id, 'Lastname': self.lastname, 'Forename': self.forename, 'Initials': self.initials,
               'Suffix': self.suffix, 'Coauthor': list(self.coauthors), 'Articles': list(self.articles),
               'Affiliation': self.Affiliation, 'FAU': self.fullname}
        JSON = json.dumps(dic, ensure_ascii=False)
        return JSON
...
def getarticle(XmlData):
    ...
    for article in xmltree.findall(".//PubmedArticle"):
        ...
        if pmid not in articlesset:
            #print pmid
            myarticle = Article(len(articlesset))
            articlesset[pmid] = myarticle
            myarticle.pmid = pmid
            NewArticleList.append(myarticle)
            ...
            for myAuthor in article.findall(u".//Author"):
                ...
                if fullname not in AuthorNameset:
                    id = len(AuthorNameset)
                    tempAuthor = Author(id)
                    AuthorNameset[fullname] = tempAuthor
                    ...
                    tempAuthor = AuthorNameset[fullname]
                changedAuthorList[fullname] = tempAuthor
                if myAuthor.find(u'Affiliation') is not None and myAuthor.find(u'Affiliation').text is not None:
                    if myAuthor.find(u'Affiliation').text not in tempAuthor.Affiliation:
                        tempAuthor.Affiliation.append(
                            myAuthor.find(u'Affiliation').text.encode('utf-8').decode('utf-8'))
            ...
            myarticle.date = {'year': ArticleDate.find('Year').text.encode('utf-8').decode('utf-8'),
                              'mon': ArticleDate.find('Month').text.encode('utf-8').decode('utf-8'),
                              'day': ArticleDate.find('Day').text.encode('utf-8').decode('utf-8')}
...
if os.path.isfile(workpath + 'Author.txt'):
    ...
    with open(workpath + 'Author.txt', 'r') as Name:
        for line in Name:
            tempaut = Author(0)
            tempaut.resume(json.loads(line.decode('utf8')))
            AuthorNameset[tempaut.fullname] = tempaut
            if tempaut.id %10000 is 0:
                print ('%d finished'%(tempaut.id))

    print ('resume Journal')
    ...
    print ('resume artical')

    with open(workpath + 'article.txt', 'r') as articlefile:
        for line in articlefile:
            tempart = Article(0)
            tempart.resume(json.loads(line.decode('utf8')))
            articlesset[tempart.pmid] = tempart
...
for path in glob.glob(filepath + '*.gz'):
    ...
    if path not in workedlist:
        ...
        with open(workpath + 'Author' + timestamp + '.txt', 'w') as authorfile:
            for eachauthor in changedworkedlist.keys():
                JSON = changedAuthorList[eachauthor].MakeJSON()
                authorfile.writelines(JSON.encode('utf8') + '\n')
            changedAuthorList.clear()
        with open(workpath + 'log.txt', 'a') as readlist:
            readlist.writelines(workpath + 'Author' + timestamp + '.txt ' + timestamp + '\n')
        ...
        with open(workpath + 'article' + timestamp + '.txt', 'w') as articlefile:
            for eacharticle in NewArticleList:
        ...
        for dele in glob.glob(workpath + 'journal*' + oldfilename + '*'):
            os.remove(dele)
